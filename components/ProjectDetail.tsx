  ...
  switch (block.type) {
      ...
      return (
        <StandardWrapper>
          <div className="max-w-3xl mx-auto">
            {block.title && (
              <h3 className="font-display text-3xl font-bold mb-6 text-zinc-900 dark:text-white">
                {block.title}
              </h3>
            )}
            <p className="text-xl text-zinc-600 dark:text-zinc-300 leading-relaxed whitespace-pre-wrap">
              {block.content}
            </p>
          </div>
        </StandardWrapper>
        ...
        <StandardWrapper>
          <div className="w-full">
            <img
              ...
              className="w-full rounded-3xl shadow-lg border border-zinc-100 dark:border-white/5"
            />
            {block.caption && (
              <p className="text-base text-zinc-500 mt-4 text-center italic">
                {block.caption}
              </p>
            )}
          </div>
            ...
            <div className="w-full max-w-5xl mx-auto">
              ...
              <div className="mt-4 flex justify-center">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-sm text-zinc-500 hover:text-brand-500 flex items-center gap-1"
                >
                  <ExternalLink size={12} /> {t("video_youtube_fallback")}
                </a>
              </div>
              {block.caption && (
                <p className="text-base text-zinc-500 mt-2 text-center italic">
                  {block.caption}
                </p>
              )}
            </div>
          ...
          <StandardWrapper>
            <div className="p-12 bg-zinc-50 dark:bg-zinc-900/50 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 text-center flex flex-col items-center justify-center">
              ...
              <h4 className="text-xl font-bold text-zinc-900 dark:text-white mb-2">
                {t("video_youtube_title")}
              </h4>
              <p className="text-zinc-600 dark:text-zinc-400 mb-6 max-w-md">
                {t("video_youtube_desc")}
              </p>
              <a
                href={block.content}
                target="_blank"
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 md:gap-8">
                {data.colors.map((color: any, idx: number) => (
                  <div key={idx} className="flex flex-col items-center">
                    ...
                    />
                    <span className="font-bold text-zinc-900 dark:text-white text-sm md:text-base text-center">
                      {color.name}
                    </span>
                    <span className="text-xs md:text-sm text-zinc-500 font-mono mt-1">
                      {color.hex}
                    </span>
                  </div>
                ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                {data.fonts.map((font: any, idx: number) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="p-6 md:p-8 bg-white dark:bg-zinc-800 rounded-3xl border border-zinc-100 dark:border-zinc-700 overflow-hidden"
                    >
                      <div className="flex justify-between items-start mb-6">
                        <h5 className="text-lg md:text-xl font-bold text-zinc-900 dark:text-white truncate pr-2">
                          {font.name}
                        </h5>
                        <span className="text-xs bg-zinc-100 dark:bg-zinc-700 px-3 py-1 rounded-full text-zinc-500 whitespace-nowrap">
                          {font.usage}
                        </span>
                      </div>
                      <div className={`${fontFamilyStyle}`}>
                        <p className="text-5xl md:text-6xl mb-6 text-zinc-900 dark:text-white">
                          Aa
                        </p>
    ...
    case "web":
      return (
        <WideWrapper>
          {block.title && (
            <h3 className="font-display text-3xl font-bold text-zinc-900 dark:text-white mb-6">
              {block.title}
            </h3>
          )}
          ...
          <div className="hidden lg:block">
            ...
            <div className="flex justify-center mt-6">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 bg-zinc-900 dark:bg-white px-8 py-3 rounded-full text-white dark:text-zinc-900 font-bold hover:opacity-90 transition-opacity shadow-lg"
              >
          ...
          <div className="block lg:hidden">
            ...
            >
              <div className="flex items-center justify-between gap-4">
                ...
                <ExternalLink
                  className="text-zinc-300 group-hover:text-brand-500 transition-colors flex-shrink-0"
                  size={20}
                />
              </div>
            </a>
...
const ProjectDetail: React.FC = () => {
  ...
  if (!project) {
    return (
      <div className="p-12 text-center text-zinc-900 dark:text-white">
    ...
    >
      <div className="w-full pb-24 pt-20 md:pt-24">
        ...
        <div className="max-w-5xl mx-auto px-6 text-center mb-16 pt-4">
          <span className="text-brand-600 dark:text-brand-400 font-bold tracking-widest uppercase text-xs md:text-sm mb-6 block">
            {project.category}
          </span>
          <h1 className="font-display text-4xl md:text-7xl font-bold text-zinc-900 dark:text-white mb-8 tracking-tight">
            {project.title}
          </h1>
          <p className="text-xl md:text-3xl text-zinc-500 dark:text-zinc-400 leading-relaxed font-light">
            {project.summary}
          </p>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-6 mb-24">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 border-y border-zinc-200 dark:border-white/10 py-12">
            ...
            <div className="text-center">
              <div className="flex items-center justify-center text-zinc-900 dark:text-white font-bold mb-3 text-lg">
                <Clock size={20} className="mr-3 text-brand-500" />{" "}
                {t("project_timeline")}
              </div>
              <p className="text-zinc-600 dark:text-zinc-300 text-lg">
                {project.timeline}
            ...
            <div className="text-center">
              <div className="flex items-center justify-center text-zinc-900 dark:text-white font-bold mb-3 text-lg">
                <Wrench size={20} className="mr-3 text-brand-500" />{" "}
                {t("project_tools")}
              </div>
              <div className="flex flex-wrap justify-center gap-2">
                {project.tools.map((tool) => (
                  <span
                    key={tool}
                    className="bg-zinc-100 dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 px-4 py-1.5 rounded-full text-sm font-medium border border-zinc-200 dark:border-zinc-800"
                  >
                    {tool}
                  </span>
                ))}
        ...
        <div className="max-w-6xl mx-auto px-6 mb-32">
          ...
          <div className="mt-16 bg-brand-50 dark:bg-brand-900/10 rounded-3xl p-10 border border-brand-100 dark:border-brand-900/30 text-center max-w-4xl mx-auto">
            <h2 className="text-xl font-bold text-brand-900 dark:text-brand-300 mb-4">
              {t("project_result")}
            </h2>
            <p className="text-brand-800 dark:text-brand-100 leading-relaxed text-xl italic">
              "{project.impact}"
            </p>
          </div>
        </div>
